...
# -*- coding: utf-8 -*-
"""
V0.5.7 块5+6 部署脚本 - Dashboard 4 widget + PWA (117 服务器)
- 推送 1 service + 1 controller + routes/api.php
- 推送前端 DashboardWidgets.vue + PwaInstallBanner.vue + index.html + main.ts + layout + public/*
- optimize:clear + restart fpm
- 烟囱: 5 widget 端点 + manifest.json + sw.js
"""
import os
import sys
import paramiko
...
WEB_FILES = [
    'src/views/dashboard/components/DashboardWidgets.vue',
...
def main():
    print('=' * 60)
    print('V0.5.7 块5+6 部署 - Dashboard widget + PWA (117)')
    print('=' * 60)
    ...
    # [2] Web src
    print(f'\n[2/6] 上传 web src {len([x for x in WEB_FILES if x.startswith("src/") or x == "index.html"])} 文件到 /tmp ...')
    for rel in WEB_FILES:
        local = os.path.join(LOCAL_WEB, rel).replace('\\', '/')
        name = os.path.basename(rel) if not rel.startswith('src/') and not rel.startswith('public/') else rel.replace('/', '_')
    ...
    # [4] sudo cp web 源文件
    print('\n[4/6] sudo cp web 源文件 + public ...')
    for rel in WEB_FILES:
        name = os.path.basename(rel) if not rel.startswith('src/') and not rel.startswith('public/') else rel.replace('/', '_')
        target = f'{REMOTE_WEB}/{rel}'
        run_remote(ssh, f'sudo -n cp /tmp/web_{name} {target}', timeout=15)
    run_remote(ssh, f'sudo -n chown -R www-data:www-data {REMOTE_WEB}/src {REMOTE_WEB}/public {REMOTE_WEB}/index.html', timeout=15)
    ...
    if os.path.exists(dist_dir):
        for root, dirs, files in os.walk(dist_dir):
            for f in files:
                ...
                if rel.startswith('assets/') or rel in ['manifest.json', 'sw.js', 'offline.html', 'favicon.svg'] or rel.startswith('icons/'):
    ...
    smoke = [
        ('GET', '/api/dashboard/widget/method-distribution'),
        ('GET', '/api/dashboard/widget/cycle-percentile'),
        ('GET', '/api/dashboard/widget/fault-top'),
        ('GET', '/api/dashboard/widget/technician-rank'),
        ('GET', '/api/dashboard/widget/all'),
        ('GET', '/manifest.json'),
        ('GET', '/sw.js'),
        ('GET', '/offline.html'),
    ...
    for m_, p in smoke:
        if p.endswith('.json') or p.endswith('.js') or p.endswith('.html') or p.endswith('.png'):
    ...
    out, _ = run_remote(ssh, "cd /var/www/oa-api && sudo -n php -d opcache.enable=0 artisan route:list --path=dashboard/widget 2>&1 | head -10", timeout=15)
    print(out[:1500])
