...
def test_finance():
    ...
    r = sess.post(f"{BASE}/expenses", headers=auth_headers(), json={
        ...
        "items": [{"description": "午餐", "amount": 380}],  # 加 items
    })
...
def test_purchase():
    ...
    # 4.2 审批采购需求（通过 purchase/approvals/{id}/decide）
    req_id = T.get("req_id")
    if req_id:
    ...
    if req_id:
        r = sess.post(f"{BASE}/purchase/plans", headers=auth_headers(), json={
            "title": f"E2E测试采购计划_{date.today()}",  # 加 title
...
def test_inventory():
    ...
    # 5.3 出库（需要先有 inventory_item_id）
    wh_id = T.get("warehouse_id")
    if wh_id:
        # 先查库存列表，找一个 inventory_item_id
        ...
        try:
            ...
            if items:
                inv_item_id = items[0].get("id")
                print(f"      🔍 找到 inventory_item_id: {inv_item_id}")
        except Exception:
            pass
        ...
        if inv_item_id:
            r = sess.post(f"{BASE}/inventory/stock-out", headers=auth_headers(), json={
                "warehouse_id": wh_id,
                "inventory_item_id": inv_item_id,  # 用正确的字段名
                "type": "outbound",
                "quantity": 5,
                "unit": "个",
                "notes": "E2E测试出库",
            })
            expect(r, "POST /inventory/stock-out", save_as="stock_out_id")
...
def test_vehicle():
    ...
    r = sess.post(f"{BASE}/vehicles", headers=auth_headers(), json={
        "plate_no": f"E2E{date.today():%m%d}测",
        "brand": "E2E测试品牌",
        "model": "E2E测试型号",
        "year": 2025,
...
def test_employee():
    p("7️⃣ 员工链路：入职→排班→考勤")
    # 7.1 入职（避免 username/email 冲突，用时间戳）
    import time
    ts = int(time.time())
...
def test_sales():
    ...
    # 8.2 创建商机（加 estimated_amount / sales_id / presale_id）
    lead_id = T.get("lead_id")
    # 先查员工列表找一个 sales_id
    r_users = sess.get(f"{BASE}/employees?page=1&page_size=5", headers=auth_headers())
    sales_id = None
    try:
        d = r_users.json().get("data", {})
        users = d.get("data", [])
        if users:
            sales_id = users[0].get("id")
            print(f"      🔍 找到 sales_id: {sales_id}")
    except Exception:
        pass

    if lead_id and sales_id:
        r = sess.post(f"{BASE}/sales/opps", headers=auth_headers(), json={
            "lead_id": lead_id,
            "name": f"E2E测试商机_{date.today():%H%M%S}",
            "estimated_amount": 120000,  # 正确的字段名
            "sales_id": sales_id,
            "presale_id": sales_id,  # 暂时用同一个
            "stage": "negotiation",
...
def test_perf():
    ...
    for label, path in endpoints:
        ...
        results.append((label, r.status_code, r.status_code == 200))


# ─── 主流程 ────────────────────────────────────────────────────
def main():
    print("🚀 端到端完整业务流程测试 V3 开始")
    print(f"   目标: {BASE}")
    ...
    print(f"  接口总数: {total}")
    print(f"  通过: {passed}/{total}  ({passed*100//total if total else 0}%)")
    print("\n  分接口结果:")
