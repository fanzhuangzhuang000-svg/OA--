#!/usr/bin/env python3
"""
Session B 阶段 3：资金和项目流转测试（服务器端运行版本）
...
def test_finance_flow():
    ...
    receivable = {
        ...
        "due_date": (datetime.now().replace(month=12, day=31)).strftime("%Y-%m-%d"),
    }
    ...
    # 4. 检查应收款状态（通过列表接口）
    print("  4️⃣ 检查应收款状态...")
    resp = auth_request("GET", "/finance/receivables")
    data = resp.json()
    
    if resp.status_code == 200 and data.get("code") == 0:
        # 在列表中查找刚创建的应收款
        receivables = data.get("data", {}).get("data", [])
        target = None
        for r in receivables:
            if r.get("id") == receivable_id:
                target = r
                break
        
        if target:
            status = target.get("status", "")
            received_amount = target.get("received_amount", 0)
            print(f"    状态: {status}, 已收金额: {received_amount}")
            
            if status in ["closed", "completed"] or received_amount == 100000:
                print(f"    ✅ 应收款已结清（无需手动关闭）")
            else:
                print(f"    ⚠️ 状态不正确，尝试手动关闭...")
                resp2 = auth_request("POST", f"/finance/receivables/{receivable_id}/close")
                data2 = resp2.json()
                if resp2.status_code == 200 and data2.get("code") == 0:
                    print(f"    ✅ 应收款关闭成功")
                else:
                    print(f"    ❌ 关闭失败: {resp2.status_code} {json.dumps(data2, ensure_ascii=False)}")
        else:
            print(f"    ⚠️ 未在列表中找到应收款 {receivable_id}")
    else:
