#!/usr/bin/env python3
"""
Session B 阶段 3 v5：全面业务流转测试（修掉所有脚本 bug）
基于 v4 结果修正：
1. 仓库响应格式判断（可能是纯数组）
2. 日期计算用 timedelta（不用字符串拼接）
"""
import requests
import json
...
def parse_list_response(resp):
    """通用：解析列表响应（支持 {code:0, data:{data:[...]}} 或 [...] 格式）"""
    try:
        d = resp.json()
...
# ════════════════════════════════════
# 6. 库存（修：仓库响应格式 + warehouse_id）
# ════════════════════════════════════
def test_inventory():
    print("\n📦 [库存] 测试...")
    code_suffix = datetime.now().strftime("%H%M%S")

    # 6.0 查仓库列表（修：支持多种响应格式）
    print("  [6.0] 查询仓库列表...")
    ...
    if items:
        ...
        print("    ⚠️ 没有仓库（需要先创建仓库）")
    # 计入测试结果
    if resp.status_code in [200, 201] and (resp.json().get("code", 0) in [0, None] if isinstance(resp.json(), dict) else True):
        results["pass"] += 1
        print("    ✅ 查询仓库列表")
    else:
        results["fail"] += 1
    ...
    if cat_id:
        ...
        # 6.3 入库（修：加 warehouse_id 和 type）
...
def test_employee_full():
    ...
    # 排班列表（带 start/end 参数）
    print("  [排班列表]（带日期参数）...")
    resp = auth_request("GET", f"/schedules?start={start_str}&end={end_str}")
    expect(resp, "查询排班列表")
    print("  ✅ 员工测试完成")
...
# ════════════════════════════════════
def main():
    print("🚀 Session B 阶段 3 v5：全面业务流转测试（修掉所有脚本 bug）")
    print("=" * 60)
