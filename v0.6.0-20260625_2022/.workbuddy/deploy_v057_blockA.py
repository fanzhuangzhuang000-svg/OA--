...
# -*- coding: utf-8 -*-
"""
V0.5.7 块A 部署脚本 - 系统初始化向导 (117 服务器)
- 推送 1 controller + routes/api.php
- 推送前端 SetupWizard.vue + router
...
WEB_FILES = [
    'src/views/settings/SetupWizard.vue',
    'src/router/index.ts',
]
...
def main():
    print('=' * 60)
    print('V0.5.7 块A 部署 - 系统初始化向导 (117)')
    print('=' * 60)
    ...
    # [1] API
    print(f'\n[1/5] 上传 API {len(API_FILES)} 文件到 /tmp ...')
    ...
    # [2] Web
    print(f'\n[2/5] 上传 web {len(WEB_FILES)} 文件到 /tmp ...')
    ...
    # [3] sudo cp + chown
    print('\n[3/5] sudo cp API+web + chown ...')
    for rel in API_FILES:
        name = os.path.basename(rel)
        target = f'{REMOTE_API}/{rel}'
        run_remote(ssh, f'sudo -n cp /tmp/{name} {target}', timeout=15)
    run_remote(ssh, f'sudo -n chown -R www-data:www-data {REMOTE_API}/app/Http/Controllers/Api {REMOTE_API}/routes', timeout=15)
    ...
    # [5] optimize:clear + restart fpm + smoke
    print('\n[5/5] optimize:clear + restart fpm + 烟囱 ...')
    ...
    out, _ = run_remote(ssh, "cd /var/www/oa-api && sudo -n php -d opcache.enable=0 artisan route:list --path=setup 2>&1 | head -15", timeout=15)
    print(out[:1500])
