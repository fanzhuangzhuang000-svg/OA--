...
# 1. login
print('=== 1. login ===')
out = run('curl -s -X POST http://127.0.0.1/api/auth/login -H "Content-Type: application/json" -d \'{"username":"admin","password":"admin123"}\' 2>&1', 15)
print('  body:', out[:200])
m = re.search(r'"token":"([^"]+)"', out)
token = m.group(1) if m else None
print(f'  token: {token[:30] if token else "NONE"}')

# 2. V0.4.1 8 budget API (字段名: item_name 而非 name)
print('\n=== V0.4.1 项目预算 (8 API) ===')
tests = []
...
# 4) POST create (字段名修正: item_name)
budget_payload = f'{{"project_id":{project_id},"title":"117部署烟囱测试预算","category":"material","total_amount":54000,"items":[{{"item_name":"线缆","category":"material","estimated_amount":24000,"quantity":120,"unit":"米","unit_price":200}},{{"item_name":"人工","category":"labor","estimated_amount":15000,"quantity":30,"unit":"工日","unit_price":500}},{{"item_name":"外包","category":"outsource","estimated_amount":15000,"quantity":1,"unit":"项","unit_price":15000}}]}}'
...
code = parts[-1]
body = parts[0]
print(f'  [3] POST create  HTTP={code} body={body[:200]}')
...
# 9) DELETE draft
budget2 = f'{{"project_id":{project_id},"title":"删除测试","category":"material","total_amount":1000,"items":[{{"item_name":"测试","category":"material","estimated_amount":1000,"quantity":1,"unit":"个","unit_price":1000}}]}}'
...
# 3. V0.4.2 路由（用正确 URL）
print('\n=== V0.4.2 供应商/外部报价/总账 (10 API) ===')
v42_tests = []
urls = [
    ('GET /suppliers', '/api/suppliers'),
    ('GET /suppliers/1', '/api/suppliers/1'),
    ('POST /suppliers/1/change-status', 'POST /api/suppliers/1/change-status'),  # GET 不支持
    ('GET /external/quote-requests', '/api/external/quote-requests'),
    ('GET /external/quote-requests/1', '/api/external/quote-requests/1'),
    ('GET /external/quote-requests/1/quotes', '/api/external/quote-requests/1/quotes'),
    ('GET /finance/supplier-payables', '/api/finance/supplier-payables'),
    ('GET /finance/customer-receivables', '/api/finance/customer-receivables'),
    ('GET /finance/summary', '/api/finance/summary'),
    ('GET /finance/aging', '/api/finance/aging'),
    ('GET /projects/suppliers', '/api/projects/suppliers'),
    ('GET /projects/1/suppliers', '/api/projects/1/suppliers'),
]
for name, url in urls:
    if name.startswith('POST'):
        out = run(f'curl -s -w "\\n%{{http_code}}" -X POST -H "Authorization: Bearer {token}" -H "Accept: application/json" -d \'{{"status":"inactive"}}\' http://127.0.0.1{url} 2>&1', 15)
    else:
...
core_urls = [
    ...
    ('GET /projects/1/construction-logs', '/api/projects/1/construction-logs'),
]
