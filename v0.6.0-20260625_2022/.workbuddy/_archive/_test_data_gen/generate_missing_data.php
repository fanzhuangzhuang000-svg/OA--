...
/**
 * 为空白表和低数据表生成测试数据
 * 使用方法：php generate_missing_data.php
 */
    ...
    for ($j = 0; $j < $numItems; $j++) {
        try {
            DB::table('quotation_items')->insert([
                ...
                'unit_price' => randomFloat(100, 5000),
                'total_price' => 0, // 会在数据库中计算
                'created_at' => now(),
                'updated_at' => now(),
            ]);
            $itemCount++;
...
for ($i = 0; $i < count($products); $i++) {
    try {
        DB::table('sales_products')->insert([
            ...
            'specification' => '标准规格',
            'unit_price' => randomFloat(500, 50000),
...
echo "8. 生成员工证书数据...\n";
$certCount = 0;
$certTypes = ['网络工程师证书', '系统集成项目管理工程师', '安防工程师证书', 'PMP', 'CISP', 'CISA'];
...
if ($targetEmp > 0) {
    for ($i = 0; $i < $targetEmp; $i++) {
        try {
            $userId = $userIds[array_rand($userIds)];
            DB::table('employee_profiles')->insert([
                'user_id' => $userId,
                'employee_no' => 'EMP' . str_pad($i + 10, 4, '0', STR_PAD_LEFT),
