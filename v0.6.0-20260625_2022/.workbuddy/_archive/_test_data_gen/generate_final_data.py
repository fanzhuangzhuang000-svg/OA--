...
for i in range(1, 31):
    ...
    status = statuses[i % 3]
    progress = (i % 10) * 10
    manager_id = user_ids[i % 4]  # 使用实际的用户 ID
...
# 2. 插入车辆使用记录 (vehicle_usage_requests)
print('\n2. 插入车辆使用记录 (50 条)...')
sql = """
...
VALUES 
"""
destinations = ['北京朝阳区', '上海浦东', '广州天河', '深圳南山', '杭州西湖', '成都锦江', '武汉洪山', '西安雁塔']
purposes = ['客户拜访', '项目现场', '会议接送', '外地出差', '日常办公', '设备运输', '应急处理', '培训会议']

values = []
for i in range(1, 51):
    vehicle_id = (i % 5) + 1
    applicant_id = user_ids[i % 4]  # 使用实际的用户 ID
    usage_date = f'2026-{(i % 6) + 1:02d}-{(i % 28) + 1:02d}'
    start_time = f'{8 + (i % 2)}:00:00'
    end_time = f'{17 + (i % 3)}:00:00'
    destination = destinations[i % 8]
...
for i in range(1, 101):
    ...
    notifiable_id = user_ids[i % 4]  # 使用实际的用户 ID
    notifiable_type = 'App\\Models\\User'
    sender_id = user_ids[i % 4] if i % 2 == 0 else 'NULL'
    level = levels[i % 4]
...
for i in range(1, 201):
    user_id = user_ids[i % 4]  # 使用实际的用户 ID
    month = (i % 6) + 1
    day = (i % 28) + 1
