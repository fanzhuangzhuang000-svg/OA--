<?php
// D:\work\website\OA\.workbuddy\quick_generate_data.php
// 快速生成测试数据 - 直接在152服务器上运行

echo "🚀 开始为152服务器生成测试数据...\n";
echo "📅 时间跨度: 2025-12-01 至 2026-06-22 (至少6个月)\n\n";

// 数据库连接配置 - 从环境变量获取
$db_host = getenv('DB_HOST') ?: '127.0.0.1';
$db_port = getenv('DB_PORT') ?: '5432';
$db_name = getenv('DB_DATABASE') ?: 'oa_db';
$db_user = getenv('DB_USERNAME') ?: 'oa_user';
$db_pass = getenv('DB_PASSWORD') ?: 'oa_pass';

// 尝试从Laravel .env文件加载配置
...
if ($item_count < 100) {
    ...
    echo "   ℹ️ 库存物品数据已足够了 ({$item_count} 条)\n";
}
...
if ($folder_count < 50) {
    ...
    echo "   ℹ️ 网盘文件夹数据已足够了 ({$folder_count} 条)\n";
}
...
if ($article_count < 50) {
    ...
    echo "   ℹ️ 知识库文章数据已足够了 ({$article_count} 条)\n";
}
