...
def login():
    r = s.post(f'{BASE}/auth/login', json={'username': 'admin', 'password': 'Oa2026Sec!'})
...
if ok:
    ...
    log('新建应收款', True, f'ID={receivable_id}')
else:
    log('新建应收款', False)
...
if ok:
    ...
    log('新建应付款', True, f'ID={payable_id}')
else:
    log('新建应付款', False)
