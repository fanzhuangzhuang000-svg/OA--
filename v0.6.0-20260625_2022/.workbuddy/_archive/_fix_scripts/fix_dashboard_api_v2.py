...
methods_with_nl = methods_text.replace('\n', nl.decode('utf-8'))

# 在 formatYuan 之前插入
marker = f"    private function formatYuan(float $amount): string{nl.decode('utf-8')}    {{{nl.decode('utf-8')}"
if marker in content:
    content = content.replace(marker, methods_with_nl + marker)
    with sftp.open('/var/www/oa-api/app/Http/Controllers/Api/DashboardController.php', 'w') as f:
        f.write(content)
    print('✓ 4 个新方法已添加（CRLF 模式）')
else:
    print('❌ 仍未找到插入点')
