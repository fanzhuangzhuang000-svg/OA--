...
TMP_PATH = '/tmp/generate_data.php'
...
def main():
    ...
    try:
        ...
        # 使用sudo复制到目标位置
        print(f"📋 正在复制脚本到 {REMOTE_PATH}...")
        cmd = f"sudo cp {TMP_PATH} {REMOTE_PATH} && sudo chown www-data:www-data {REMOTE_PATH}"
        ...
        # 执行PHP脚本 - 使用www-data用户运行
        cmd = f"cd /var/www/oa-api && sudo -u www-data php {REMOTE_PATH}"
        ...
        # 清理临时文件
        ssh.exec_command(f"sudo rm -f {TMP_PATH} {REMOTE_PATH}")
        print(f"🗑️  已清理临时文件")
