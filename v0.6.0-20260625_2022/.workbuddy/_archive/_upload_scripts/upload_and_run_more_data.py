...
def main():
    ...
    try:
        ...
        with open('D:/work/website/OA/.workbuddy/generate_more_data.php', 'r', encoding='utf-8') as f:
            php_script = f.read()
        ...
        # 上传base64编码的脚本
        ssh.exec_command(f'echo "{encoded_script}" | base64 -d > /tmp/generate_more_data.php')
        time.sleep(3)
        
        # 检查文件是否上传成功
        stdin, stdout, stderr = ssh.exec_command('ls -lh /tmp/generate_more_data.php')
        ...
        stdin, stdout, stderr = ssh.exec_command('php /tmp/generate_more_data.php')
...
try {
    $expenseStats = DB::table('expense_claims')
        ->selectRaw('MIN(created_at) as earliest, MAX(created_at) as latest, COUNT(*) as total')
        ->first();
...
"""
        ...
        ssh.exec_command(f'echo "{encoded_check}" | base64 -d > /tmp/check_final_data.php')
        time.sleep(2)
        
        # 运行检查脚本
        stdin, stdout, stderr = ssh.exec_command('php /tmp/check_final_data.php')
        time.sleep(5)
        ...
        # 5. 清理临时文件
        ssh.exec_command('rm /tmp/generate_more_data.php /tmp/check_final_data.php')
        
        ssh.close()
        print("\n✅ 测试数据生成完成")
        print("\n⚠️ 注意: 如果某些模块的数据量仍然很少，可能需要手动生成更多数据")
