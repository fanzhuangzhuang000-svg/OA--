...
try:
    ...
    print("\n上传 SQL 文件...")
    sftp = ssh.open_sftp()
    local_file = r'D:\work\website\OA\.workbuddy\test_data_correct.sql'
    remote_file = '/tmp/test_data_correct.sql'
    ...
    while True:
        ...
        if not line:
            break
        output += line
        if 'INSERT' in line or 'ON CONFLICT' in line or 'COMMIT' in line or 'ERROR' in line:
            print(line.strip())
    ...
    print("\n" + "=" * 60)
    print("✅ 测试数据生成完成！")
    print("\n下一步: 为其他表（leads, opportunities, attendance_records 等）生成数据")
