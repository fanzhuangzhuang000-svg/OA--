...
def main():
    ...
    try:
        ...
        with open('D:/work/website/OA/.workbuddy/generate_simple_data.php', 'r') as f:
            php_script = f.read()
        ...
        # 上传base64编码的脚本
        ssh.exec_command(f'echo "{encoded_script}" | base64 -d > /tmp/generate_simple_data.php')
        time.sleep(3)
        
        # 检查文件是否上传成功
        stdin, stdout, stderr = ssh.exec_command('ls -lh /tmp/generate_simple_data.php')
...
"""
        ...
        # 5. 清理临时文件
        ssh.exec_command('rm /tmp/generate_simple_data.php /tmp/check_data.php')
