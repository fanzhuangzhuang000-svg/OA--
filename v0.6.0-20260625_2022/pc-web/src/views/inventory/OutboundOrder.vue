  ...
  <div class="page-container">
    ...
    <div class="filter-bar">
      <el-select v-model="filterType" placeholder="出库类型" clearable style="width:160px" @change="loadList(1)">
        <el-option label="领用出库" value="outbound" /><el-option label="销售出库" value="sale" /><el-option label="报废出库" value="scrap" />
      </el-select>
    ...
    <div class="content-card">
      <el-table v-loading="loading" :data="list" stripe border style="width:100%">
        ...
        <el-table-column label="快递公司" width="90" show-overflow-tooltip>
          <template #default="{row}">{{ row.logistics_company||'-' }}</template>
        </el-table-column>
        <el-table-column label="快递单号" width="140">
          <template #default="{row}"><span class="record-no">{{ row.logistics_no||'-' }}</span></template>
        </el-table-column>
        ...
        <el-table-column label="出库数量" width="100" align="right">
          <template #default="{row}"><span style="font-weight:600;color:#A32D2D">-{{ row.quantity }}</span><span class="unit-text"> {{ row.inventoryItem?.unit||'' }}</span></template>
        </el-table-column>
        <el-table-column label="出库后库存" width="100" align="center">
    ...
    <el-dialog v-model="showFormDialog" title="新增出库单" width="1500px" :close-on-click-modal="false" top="3vh">
          ...
          <el-row :gutter="16">
            ...
            <el-col :span="8">
              <el-form-item label="出库类型">
                <el-radio-group v-model="form.type">
                  <el-radio value="outbound">领用</el-radio>
                  <el-radio value="sale">销售</el-radio>
            ...
            <el-col :span="8">
              <el-form-item label="快递公司">
                <el-select v-model="form.logistics_company" filterable allow-create clearable placeholder="选择快递公司" style="width:100%">
            ...
            <el-col :span="8">
              <el-form-item label="快递单号">
                <el-input v-model="form.logistics_no" placeholder="请输入快递单号" clearable />
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="16">
      ...
      <div class="section-card" style="margin-top:16px">
        <div class="section-title" style="display:flex;align-items:center;justify-content:space-between">
          <span><el-icon><Goods /></el-icon> 出库明细</span>
...
function typeLabel(t) { return { outbound:"领用", sale:"销售", scrap:"报废" }[t]||t }
const autoRecordNo = computed(() => {
  const d = new Date()
  const pad = n=>n.toString().padStart(2,"0")
  return "CK-"+d.getFullYear()+pad(d.getMonth()+1)+pad(d.getDate())+"-XXXX"
})
async function loadList(page=1) {
  ...
  try {
    ...
    if (!filterType.value) items = items.filter(r=>["outbound","sale","scrap"].includes(r.type))
    if (searchKey.value) {
...
async function handleSubmit() {
  ...
  try {
    ...
    ElMessage.success("出库完成，共 "+successCount+" 项物料成功"+(lastRecordNo?"，单号："+lastRecordNo:""))
    showFormDialog.value = false
    loadList(pagination.page)
  } catch(e) {
    if (successCount > 0) {
      ElMessage.warning("已成功出库 "+successCount+" 项，部分失败")
    } else {
...
onMounted(()=>{ loadList(1); loadItems(); loadWarehouses(); loadCustomers(); loadProjects() })
</script>
<style scoped>
