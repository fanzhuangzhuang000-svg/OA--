...
const filter = reactive({ keyword: '', subType: '', stage: '', dateRange: [] as any[] })
...
const stageLabel = (s: string) => stageOptions.find(x => x.value === s)?.label || s
...
const createPending = computed(() => list.value.filter(i => i.status === 'pending' && i.current_approver === 'me' && i.subType === 'project_create').length)
const stagePending = computed(() => list.value.filter(i => i.status === 'pending' && i.current_approver === 'me' && i.subType === 'project_stage').length)
const monthlyCreated = computed(() => list.value.filter(i => i.subType === 'project_create' && i.status === 'approved').length)

const loadList = async () => {
  loading.value = true
  try {
    const res: any = await get('/approvals/project', { per_page: 200 })
...
const handleApprove = async (row: any) => { try { await ElMessageBox.confirm('确定审批通过吗？', '审批确认', { type: 'success' }); await post(`/approvals/project/${row.id}/approve`, { comment: approvalComment.value || '同意' }); ElMessage.success('已通过'); showDetailDialog.value = false; loadList() } catch (e: any) { if (e !== 'cancel') ElMessage.error(e?.message || '操作失败') } }
const handleReject = async (row: any) => { if (!approvalComment.value.trim()) { ElMessage.warning('请填写驳回意见'); return }; try { await ElMessageBox.confirm('确定驳回吗？', '驳回确认', { type: 'warning' }); await post(`/approvals/project/${row.id}/reject`, { comment: approvalComment.value }); ElMessage.success('已驳回'); showDetailDialog.value = false; loadList() } catch (e: any) { if (e !== 'cancel') ElMessage.error(e?.message || '操作失败') } }
