...
const subTypeOptions = [
  ...
  { value: 'purchase', label: '采购' }, { value: 'purchase_payment', label: '采购付款' },
...
const subTypeTagType = (t: string): any => ({
  ...
  purchase: 'success', purchase_payment: 'success', sales: 'success', discount: 'warning',
...
const subTypeIcon = (t: string) => ({
  ...
  purchase: 'ShoppingCart', purchase_payment: 'CreditCard', sales: 'TrendCharts', discount: 'Discount',
...
const leavePending = computed(() => list.value.filter(i => i.status === 'pending' && i.current_approver === 'me' && i.subType === 'leave').length)
const vehiclePending = computed(() => list.value.filter(i => i.status === 'pending' && i.current_approver === 'me' && i.subType === 'vehicle').length)
const purchasePending = computed(() => list.value.filter(i => i.status === 'pending' && i.current_approver === 'me' && i.subType === 'purchase').length)

const loadList = async () => {
  loading.value = true
  try {
    const res: any = await get('/approvals/operation', { per_page: 200 })
...
const handleApprove = async (row: any) => { try { await ElMessageBox.confirm('确定审批通过吗？', '审批确认', { type: 'success' }); await post(`/approvals/operation/${row.id}/approve`, { comment: approvalComment.value || '同意' }); ElMessage.success('已通过'); showDetailDialog.value = false; loadList() } catch (e: any) { if (e !== 'cancel') ElMessage.error(e?.message || '操作失败') } }
const handleReject = async (row: any) => { if (!approvalComment.value.trim()) { ElMessage.warning('请填写驳回意见'); return }; try { await ElMessageBox.confirm('确定驳回吗？', '驳回确认', { type: 'warning' }); await post(`/approvals/operation/${row.id}/reject`, { comment: approvalComment.value }); ElMessage.success('已驳回'); showDetailDialog.value = false; loadList() } catch (e: any) { if (e !== 'cancel') ElMessage.error(e?.message || '操作失败') } }
const handleTransfer = async (row: any) => { try { const { value: target } = await ElMessageBox.prompt('请输入转交给谁', '转交审批', { inputPlaceholder: '用户名' }); if (!target) return; await post(`/approvals/operation/${row.id}/forward`, { target }); ElMessage.success(`已转交 ${target}`); showDetailDialog.value = false; loadList() } catch (e: any) { if (e !== 'cancel') ElMessage.error(e?.message || '操作失败') } }
const handleSearch = () => {}
